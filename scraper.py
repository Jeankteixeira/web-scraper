...
try:
    ...
    print(f"Warning: Scoring modules not available: {e}")
    SCORING_AVAILABLE = False

# Import our new scoring modules
try:
    from scoring_engine import LinkedInProfileScorer
    from profile_processor import ProfileProcessor
    SCORING_AVAILABLE = True
...
# Módulo de Busca de Artigos Acadêmicos - Apenas Bases Gratuitas
def search_academic_articles(keywords, sources=None, publication_year=None, authors=None, 
    ...
    if not keywords or not keywords.strip():
        raise ValueError("Erro: O campo 'palavras-chave' é obrigatório para realizar a busca.")
    
    if len(keywords.strip()) < 2:
        raise ValueError("Erro: As palavras-chave devem ter pelo menos 2 caracteres.")
    
    keywords = keywords.strip()
    results = []
    
    # Definir fontes gratuitas permitidas
    free_sources = ['google-scholar', 'arxiv', 'scielo', 'dblp']
    
    # Se não especificado, buscar em todas as fontes gratuitas
    if sources is None:
        sources = free_sources
    else:
        # Filtrar apenas fontes gratuitas
...
def build_enhanced_query(keywords, authors=None, institution=None, knowledge_area=None, language=None):
    """Constrói uma query aprimorada com filtros adicionais"""
    query_parts = [keywords]
    
    if authors:
        # Adicionar autores à query
...
def apply_post_search_filters(results, publication_year=None, authors=None, language=None, 
    ...
    """Aplica filtros aos resultados após a busca"""
...
def filter_by_year(result, start_year, end_year):
    """Filtra resultado por ano de publicação"""
    if 'published' in result:
        ...
        try:
            ...
            pass
    return True  # Se não conseguir determinar o ano, inclui o resultado

def filter_by_language(result, language):
    """Filtra resultado por idioma"""
...
def filter_by_institution(result, institution):
    """Filtra resultado por instituição"""
...
def search_scielo(keywords, language=None):
    ...
    try:
        ...
        # Mapear idioma para parâmetro do SciELO
        lang_param = 'pt'  # padrão
        if language == 'english':
            lang_param = 'en'
...
def search_dblp(keywords):
    """Busca artigos no DBLP Computer Science Bibliography"""
    results = []
    try:
        encoded_keywords = urllib.parse.quote_plus(keywords)
...
# Módulo de Raspagem do LinkedIn com Scoring
def scrape_linkedin_profiles(keywords, location=None, job_seniority=None, experience_ranges=None, 
    ...
    """
    Busca perfis do LinkedIn com filtros avançados e scoring de relevância
    
    Args:
        keywords: Palavras-chave para busca
        location: Localização (ex: "Brasil", "São Paulo")
        job_seniority: Nível do cargo (ex: "senior", "junior")
        experience_ranges: Lista de faixas de experiência (ex: ["2-5", "5-10"])
        industries: Lista de indústrias (ex: ["tecnologia", "financeiro"])
        technical_skills: Lista de habilidades técnicas
        languages: Lista de idiomas
        company: Empresa atual/anterior
        education: Nível de educação
        certifications: Lista de certificações
        enable_scoring: Se deve aplicar scoring de relevância
    """
    
    # Para demonstração, vamos usar dados mock mais realistas
    # Em produção, isso seria substituído por scraping real do LinkedIn
    ...
    # Apply scoring if available and enabled
...
def generate_mock_linkedin_profiles(keywords, location, job_seniority, experience_ranges, 
    ...
    """Gera perfis mock baseados nos filtros fornecidos com dados mais ricos para scoring"""
    ...
    # Filtro por habilidades técnicas
    if technical_skills:
        filtered_profiles = [p for p in filtered_profiles 
...
# Função para testar o sistema de scoring
def test_scoring_system():
    """Testa o sistema de scoring com perfis de exemplo"""
    ...
    for profile in scored_profiles[:5]:  # Mostrar top 5
...
def main():
    ...
    if choice == "1":
        # Busca de artigos acadêmicos com validação
        while True:
            ...
            try:
                ...
                for i, article in enumerate(articles[:5], 1):  # Mostra apenas os primeiros 5
        ...
        # Busca de perfis LinkedIn com scoring
